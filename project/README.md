...
3. Structured Results - Each agent returns a Pydantic model with specific flags (e.g., contains_pii, is_unfriendly) plus a rationale. The definitions are in the the `types/moderation_result.py` folder.
...
The Video and Audio agents are already completed. You can test them with `uv run pytest tests/test_video_agent.py` and `uv run pytest tests/test_audio_agent.py`.
